import moment from 'moment';

const toString = Object.prototype.toString;

/**
 * 判断value是否为数字
 */
export const isNumber = (value) => {
  return typeof value === 'number' || toString.call(value) === '[object Number]';
}

/**
 * 获取服务器当前时间
 */
export const getServerCurrentTime = () => {
  const { timeOffest } = window.g_app._store.getState().global;
  return moment(new Date()).add(timeOffest, 'milliseconds');
};


/**
 * 小数转换为百分数
 */
export const toPercent = (number, dec = 0) => {
  return `${Number(number * 100).toFixed(dec)}%`;
};

/**
 * 空状态图片
 */
export const emptyPicture = () => {
  return "data:image/svg+xml;base64,";
};

export const convertMem = (size) => {
  return Number.isNaN(size) ? 0 : (size / 1024).toFixed(2) * 100 / 100;
};

/**
 * 获取 resource min max step值
 */

export const getResourceTypes = (resource = {}) => {
  const { cpus, mem, gpus } = resource;
  const resourceFormat = (resourceType = {}, extendFn) => {
    return extendFn ? {
      min: extendFn(resourceType.min),
      max: extendFn(resourceType.max),
      step: extendFn(resourceType.step),
    } : {
      min: resourceType.min,
      max: resourceType.max,
      step: resourceType.step,
    };
  };
  return {
    cpu: { title: 'CPU(Coress)', ...resourceFormat(cpus) },
    mem: { title: 'MEM(GB)', ...resourceFormat(mem, convertMem) },
    gpu: { title: 'CPU(Coress)', ...resourceFormat(gpus) },
  };
};

export const defaultResourceType = ['cpu','mem','gpu']
